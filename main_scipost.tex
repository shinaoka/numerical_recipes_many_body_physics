...
we can analytically continute $G(\iv)$ to the whole complex plane as $G(z)$ for $z\in \mathbb{C}$.
...
we may want to evaluate it on abitrary points.
...
Using your favarite quadrature rule (and interpolation if nescessary),
...
Another tricky thing is inverse Fourer transform of $G(\iv)$ to $G(\tau)$.
As we have already seen in Exercise~\ref{ex:native-inverse-transform},
the naive inverse Fourier traformation does not work at all.
...
we can substract the first term of the expansion from $G(\iv)$ as
\begin{align}
...
\begin{Exercise}
    Implement Eq.~\eqref{eq:inverse-transform-c0} for the Hubbard model and confirm that the discontinuity at $\tau=0$ is correctly reproduced.
\end{Exercise}
