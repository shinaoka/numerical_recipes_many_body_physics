...
In the original proposal of the IR basis, they proposed to use different basis functions for bosons.
In this lecture, following a recent proposal in a follow-up paper, 
...
\end{align}
where $\rho'(\omega)$ is a smooth function linearly vanishing at $\omega=0$ and $\epsilon$ is an infinitesimal number.
...
At low frequencies, we obtain
\begin{align}
   \tilde \rho(\omega) &\simeq \left(\frac{2\rho_0\epsilon}{\omega}\right) \delta(\omega-\epsilon) + \frac{2\rho_1(\omega)}{\beta\omega},
\end{align}
where the second term is a smooth function around $\omega=0$.
Substituting Eq.~\eqref{eq:rho-split} into Eq.~\eqref{eq:gtau-boson} yields
\begin{align}
...
The singular term, i.e., the constant term in $\tau$, must be treated seperately if any.
