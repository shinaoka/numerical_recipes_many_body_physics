...
\begin{align}
	...
	G(\tau-\tau') &= -\theta(\tau-\tau')\expval{A(\tau)B(\tau')\pm B(\tau')A(\tau)}.
\end{align}
...
We used the formula
\begin{align}
   T\sum_{\nu} \frac{e^{-\iv\tau}}{\iv - \epsilon} &= \frac{e^{-\epsilon\tau}}{1+e^{-\beta\epsilon}}.
\end{align}

Figure~\ref{fig:hubbard-atom} plots Eqs.~\eqref{eq:gz-hubbard-atom} and \eqref{eq:gtau-hubbard-atom} for several different values of $\beta$.
One can see that $G(\tau)$ changes very rapidly around $\tau=0$ and $\beta$ at large $\beta$.
Due to the sum rule and the half-filling condition,
$G_{\sigma\sigma'}(0^+)=G_{\sigma\sigma'}(\beta+0^-) = -1/2$.
On the other hand, $G(\iv)$ decays only algebratically at high frequencies.


\clearpage
\section{Conventional numerical tools for Green's function}
\subsection{Uniform mesh}
Due to the algebratical decay of $G(\iv)$ and the strong $\tau$ dependence of $G(\tau)$ around $\tau=0$,
