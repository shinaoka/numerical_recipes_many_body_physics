...
\begin{align}
    G(\tau) &= - \int_{-\omega_\mathrm{max}}^{\omega_\mathrm{max}}d\tau K^\mathrm{F}(\tau, \omega) \rho(\omega),
\end{align}
where
$$
...
($[-\omega_\mathrm{max}, \omega_\mathrm{max}]$ and $[0,\beta]$).
...
\end{align}
where $\boldsymbol{U}$ and $\boldsymbol{V}$ are matrices whose columns consist of singular vectors
