...
%\textcolor{red}{Write it!}
The basis functions in Matsubara frequencies are defined as
\begin{align}
...
You can install \texttt{sparse-ir} by running the following command from a shell:
\begin{verbatim}
$ pip install -U sparse-ir xprec
\end{verbatim}
...
\begin{figure}
    ...
    \caption{
        Singular values and IR basis functions computed by \texttt{sparse-ir} for $\Lambda=100$ and $\beta=10$.
...
We demonstrate how to do this transform using \texttt{sparse-ir} for the spectral function consisting 
...
%This functionality is implemented in \texttt{sparse-ir}, which can be used as follows:
...
An empiritically good choice is to use the middle points of the neighboring roots of 
...
smpl = sparse_ir.TauSampling(basis, sampling_points=my_smpl_points)
\end{python}
\end{Exercise}
...
\begin{figure}
    ...
    with $U_{29}(\iv)$.
    %The lower panel is an enlargement of the upper panel at low frequencies.
    }
...
\begin{Exercise}
    ...
    my_smpl_points = ***
    smpl_matsu = sparse_ir.MatsubaraSampling(basis, sampling_points=my_smpl_points)
    \end{python}
\end{Exercise}
...
%As knonw, the model exhibits a transition to the antiferromagnetic order with 
%the wave vector $\bm{Q}=(\pi,\pi)$ shown in Fig.~\ref{fig:antiferro}.
%Thus, we consider two sublattices A and B shown in Fig.~\ref{fig:antIiferro}(b).
%
%At half filling $\mu = U/2$,
%A Hartree-Fock self-consistent equation for this model reads
%\begin{align}
    %n_\sigma &= \frac{1}{N}\sum_{\bk} f(\epsilon_{\bk\sigma}),\\
    %\epsilon_{\bk\sigma} &= \epsilon_{0,\bk} + U n_{\bar \sigma} - \mu.
%\end{align}
%where $f(\epsilon) = \frac{1}{1+e^{\beta \epsilon}}$.
%Self-consistent calculations can be peformed efficiently without using Green's functions.
%The following is a sample Python code.
%%
%Figure~\ref{fig:hf} shows results computed by the code for $U=***$.
%The system shows an antiferromagnetic magnetic transition at $T_\mathrm{c} \simeq ***$.
%
%Although the Hartree-Fock calculations can be peformed most efficiently 
%As a toy example, let us implement the self-consistent equations using Green's functions.
%The electron density can be computed by
%\begin{align}
    %G(\iv, \bk) &= \frac{1}{N}\sum_{\bk} \frac{1}{\iv -  \epsilon_{\bk\sigma}}.
%\end{align}
%
%
%\clearpage
...
%you have learned how to store numerical data of Matsubara Green's functions
%and transform them between different domains efficiently.
%You are now ready to learn how to solve a diagrammatic equations
%in this and subsequent sections.
%In this section,
%as a simple example, we calculate the self-energy of the two-dimensional Hubbard model within the range of second-order perturbations.
%
%
%We assume the band dispersion
%\begin{align}
    %\epsilon(k) &= -2 (\cos{k_1} + \cos{k_2}),
%\end{align}
%where $k_1$ and $k_2$ are momenta.
...
%We consider an onsite interaction,
%\begin{align}
  %& \sum_r U n_{\uparrow}(r) n_{\downarrow}(r)
%\end{align}
%https://repository.kulib.kyoto-u.ac.jp/dspace/bitstream/2433/97757/1/KJ00004705688.pdf
%\begin{figure}
    %\centering
...
%In the Matsubara-frequency domain, the second-order self-energy reads
%\begin{align}
   %A & B 
%\end{align}
...
\begin{figure}
    ...
    \end{minipage}
    %\centering
    %\begin{flushleft}\hspace{8em}(a)\end{flushleft}
%
    %\begin{flushleft}\hspace{8em}(b)\end{flushleft}
    %\includegraphics[width=0.5\columnwidth]{second_order_sigma_r.pdf}
%
    %\begin{flushleft}\hspace{8em}(c)\end{flushleft}
    %\includegraphics[width=0.5\columnwidth]{second_order_sigma_k_ir.pdf}
%
    %\begin{flushleft}\hspace{8em}(d)\end{flushleft}
    %\includegraphics[width=0.5\columnwidth]{second_order_sigma_iv.pdf}
...
%
%This Fourier transform can be performed efficiently using the FFT algorithm.
%The one-dimensional (inverse) discrete Fourier transform are defined as
%\begin{align}
    %y_m &= \frac{1}{N} \sum_{n=0}^{N-1} x_n e^{\frac{2\pi n m}{N}\ii},\\
    %x_n &= \sum_{m=0}^{N-1} x_m e^{-\frac{2\pi n m}{N}\ii}.
%%\end{align}
%%where IFFT denotes inverse FT.
%Two- and three-dimensional discrete are defined in a similar way.
%
%
%First, for $\beta = 10^3$ and $\wmax = 10^2$ ($\Lambda=10^5$), 
%we construct a basis object (with 138 basis functions).
%The generation of sampling points and the computation of $\hatFmat_\alpha$ and $\Fmat_\alpha$ as well as their pseudo-inverse matrices are done by \texttt{MatsubaraSampling} and \texttt{TauSampling}.
